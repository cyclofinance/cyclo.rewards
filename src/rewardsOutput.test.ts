...
function parseCsv(filePath: string): {
  header: string;
  entries: Array<{ address: string; reward: bigint }>;
} {
...
function parseBalancesCsv(filePath: string): {
  header: string;
  columns: string[];
  addresses: string[];
} {
...
describe("current epoch rewards output", () => {
  ...
  it("no CYTOKEN addresses in rewards", () => {
    for (const token of CYTOKENS) {
      ...
      expect(rewardAddresses.has(token.underlyingAddress.toLowerCase())).toBe(
        false,
      );
      expect(rewardAddresses.has(token.receiptAddress.toLowerCase())).toBe(
        false,
      );
    }
  });
...
describe("balances arithmetic consistency", () => {
  ...
  function parseRow(line: string) {
    ...
    return {
      address,
      tokenData,
      totalRewards: BigInt(parts[parts.length - 1]),
    };
  }
  ...
  it("total_rewards equals sum of per-token rewards for all accounts", () => {
    for (const row of allRows) {
      const sum = CYTOKENS.reduce(
        (s, t) => s + row.tokenData[t.name].rewards,
        0n,
      );
...
describe("cross-epoch comparison", () => {
  ...
  const currentShares = new Map(
    currentEntries.map((r) => [
      r.address,
      Number((r.reward * 10000n) / currentTotal) / 10000,
    ]),
  );
  const prevShares = new Map(
    prevEntries.map((r) => [
      r.address,
      Number((r.reward * 10000n) / prevTotal) / 10000,
    ]),
  );

  function getFlaggedChanges(): Array<{
    address: string;
    prevShare: number;
    currentShare: number;
    change: string;
      ...
      if (prev === 0) {
        ...
        if (relativeChange > 0.5) {
          changes.push({
            ...
            prevShare: prev,
            currentShare: curr,
            change: `${(relativeChange * 100).toFixed(0)}%`,
          });
        }
      }
    }
